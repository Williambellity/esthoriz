...
class ContacterController extends WController {
	/*
			...
			if (!$resp->is_valid) {
				...
				if (!empty($erreurs)) {
					...
					if (!$this->model->createMail($mailData)) {
						...
						$mail = new PHPMailer();
						$mail->CharSet = 'utf-8';
						$mail->From = 'contacter@est-horizon.com';
						$mail->FromName = 'Forum Est-Horizon';
						$mail->Subject = 'Votre demande de contacter pour le Forum Est-Horizon';
						$mail->Body = "<h1>Votre demande de contacter pour le Forum Est-Horizon</h1>
