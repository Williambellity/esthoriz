...
class NewsletterAdminModel {
	...
	public function getVisiteurs(){
		//Ne cible que les visiteurs inscrits ayant autorisé la newsletter et ceux inscrit à la newsletter
		$prep = $this->db->prepare('
			SELECT email
			FROM newsletter
			WHERE mailingactif = 1
			ORDER BY email ASC
		');
		$prep->execute();
		return $prep->fetchAll(PDO::FETCH_ASSOC);		
	...
	public function getTest() {
		// Pour les tests
		return array(array('email' => 'l.nicaud@eleves-alsacienne.org'));
		// $prep = $this->db->prepare('
			// SELECT mail
			// FROM visiteurs
		// ');
	...
	public function getRegistered() {
		...
		$prep = $this->db->prepare('
			...
			WHERE entreprises.id = entreprises_registered.firmid AND entreprises_contacts.id = entreprises_registered.contactid AND userid<>0 AND phoning_step = 6 AND entreprises_registered.date BETWEEN \'2020-01-01\' AND \'2020-12-31\'
			ORDER BY name ASC
		');
		$prep->execute();
		return $prep->fetchAll(PDO::FETCH_ASSOC);
	...
	public function getWithoutStand() {
		$prep = $this->db->prepare('
			SELECT email
			FROM entreprises, entreprises_contacts, entreprises_registered
			WHERE entreprises.id = entreprises_registered.firmid AND entreprises_contacts.id = entreprises_registered.contactid AND userid<>0 AND phoning_step = 6 AND entreprises_registered.date BETWEEN \'2020-01-01\' AND \'2020-12-31\' AND entreprises.id NOT IN (Select firmid From logistic_firm)
			ORDER BY email ASC
		');
		$prep->execute();
		return $prep->fetchAll(PDO::FETCH_ASSOC);
	...
	public function getWithoutBrochure() {
		...
		$prep = $this->db->prepare('
			SELECT email
			FROM entreprises, entreprises_contacts, entreprises_registered
			WHERE entreprises.id = entreprises_registered.firmid AND entreprises_contacts.id = entreprises_registered.contactid AND userid<>0 AND phoning_step = 6 AND entreprises_registered.date BETWEEN \'2020-01-01\' AND \'2020-12-31\' AND entreprises.id NOT IN (Select firmid From brochure)
			ORDER BY email ASC
		');
		$prep->execute();
		return $prep->fetchAll(PDO::FETCH_ASSOC);
