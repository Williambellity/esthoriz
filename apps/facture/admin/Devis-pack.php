...
function commande($fichier){
	...
	$this->Ln();
	$this->Cell(0,10,"Facture à retourner datée et signée avec la mention « Bon pour accord »",0,1,'L');
...
function facture($fichier){
    ...
    // Titre
	$this->SetFont('Arial','',16);
    $this->Cell(0,6,"DEVIS",0,1,'C');   
    $this->Ln();
    // Numéro de commande 
	$this->Cell(0,3,'Numéro du devis (notre référence): '.$annee.' - '. $_POST["no"] ,0,1,'C');
    // generation time
    $this->Ln();
    $this->SetFont('Arial','',10);
    $BDCtext = "";
    if(intval($_POST["BDCCode"])!=0||$_POST["BDCCode"]!=""){
        $BDCtext = "No du devis : ".$_POST["BDCCode"]." | ";
    }
