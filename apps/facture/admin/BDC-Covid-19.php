...
function lettre($fichier){
    ...
    $this->Ln();
	$this->MultiCell(0,5,"Par ailleurs, la signature de ce bon de commande annule celui que vous aviez précédemment envoyé (Commande n°2020 - ".utf8_decode($_POST["no"]).") pour votre participation inscription à l’édition qui devait avoir lieu le 19 novembre au Centre Prouvé.\n\nDans l’attente de votre réponse, nous vous prions d’agréer l’expression de nos salutations distinguées.");
    // Saut de ligne
    $this->Ln();
}
...
function inscription($fichier){
    ...
    array_push($data,array("Responsable",utf8_decode($_POST["poste"])),
...
function commande($fichier){
	...
	//text
	$this->SetFont('Arial','B',11);
	$this->Cell(0,10,'Forum Est-Horizon, '.$date.' sur '.$lieu.'',0,1,'C');
	...
	/* Data construction */
	$data = array(
				array(utf8_decode('Pack '.$_POST["pack"]),$_POST["prix"],$_POST["packno"],$_POST["prix"])
				);
    ...
    if(intval($_POST["repas"])!=0){
        $data[] = array("Repas supplémentaires",$_POST["repasprix"],utf8_decode($_POST["repas"]),$_POST["repasprix"]*utf8_decode($_POST["repas"]));
...
function facture($fichier){
    ...
    $this->SetFont('Arial','',11);
	 ...
	 $data = array(
				...
				array("Responsable",utf8_decode($_POST["poste"])),
				array("Numéro de téléphone",$_POST["tel"]),
				array("Email",utf8_decode($_POST["email"])),
				);
	 $this->Table($w, $header, $data);
	...
	/* Data construction */
	$data = array(
				array(utf8_decode('Pack '.$_POST["pack"]),$_POST["prix"],"1",$_POST["prix"])
				);
	$total = $_POST["prix"];
	$data[]= array("TOTAL HT","","",$total);
	$data[]= array("TOTAL TTC(*)","","",$total);	
...
$pdf->Output("../../tresorerie/admin/facturesperso/".$_POST["name"].".pdf","F");
$pdf->Output();
