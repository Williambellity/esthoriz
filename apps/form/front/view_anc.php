...
class EtudiantsFrontView extends WView {
    private $model;
	
    public function __construct(EtudiantsModel $model) {
        parent::__construct();
		$this->model = $model;
    }
	...
	public function liste($data, $display_ecole) {
		$paymods = array('Indéfini', 'Chèque', 'Liquide', 'Exonéré');
		$bus_persons = array();
		foreach ($data as $p) {
			$p['pay_mod'] = $paymods[$p['pay_mod']];
			
			if (isset($bus_persons[$p['bus']])) {
				$bus_persons[$p['bus']][] = $p;
			} else {
				$bus_persons[$p['bus']] = array($p);
			}
		}
