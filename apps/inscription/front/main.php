...
class InscriptionController extends WController {
		...
		if ($data['nom']!=NULL){//!in_array(null, $data, true)) {
			...
			if (!empty($erreurs)) {
				...
				if ($this->model->writeInscription($data)) {
					...
					WNote::info(utf8_encode("Demande d'inscription realisee"), utf8_encode(" Vous allez recevoir un mail 
	...
	public function confirmation() {
		...
		if (!empty($email)) {
			...
			{
			WNote::success(utf8_encode("Inscription valid�e"), utf8_encode("Nous vous donnons rendez-vous le Jeudi 2 Decembre 2021 au Centre Prouv�.<br /><br />
