...
class FdhAdminController extends WController {
    ...
    public function __construct() {
        if ($_SESSION['accessString'] == 'all' || in_array($_SESSION['userid'], $this->authorizedUsers)) {
            ...
            $this->actionList = array(
                ...
                'liste' => "Liste des inscrits au départ",
                'del_entry' => "\Suppression d'un utilisateur",
                'printaller' => "Imprimer la liste des inscrits au départ",
                'printretour' => "Imprimer la liste des inscrits au retour",
                'bus' => "Gestion des bus",
                'bde_manager' => "Gestion des BDE",
                'purge' => "Vider les données"
            );
		}
    ...
    public function main() {
		...
		if (!in_array(null, $data, true)) {
			...
			if (empty($todo)) { // Il n'y a pas eu d'erreurs
				...
				foreach ($bus_full as $idballer => $candidates) {
				...
				foreach ($bus_full as $idbretour => $candidates) {
    ...
    public function printaller(){
        $this->view->printaller();
        $this->render('printaller');
    }
    
    public function printallerbus(){
		...
		if (!in_array($_SESSION['userid'], $this->authorizedUsers) && !in_array('all', $_SESSION['access'])) {
			...
			$data = $this->model->listallerByBus($bus);
		}
    ...
    public function printretourbus(){
		...
		if (!in_array($_SESSION['userid'], $this->authorizedUsers) && !in_array('all', $_SESSION['access'])) {
			...
			$data = $this->model->listretourByBus($bus);
		}
 		//impression dans excel
		require('helpers/excelPHP/PHPExcel.php');
		$excel = new PHPExcel();
    ...
    public function mn_printaller(){
        $data = array(array());
		if (!in_array($_SESSION['userid'], $this->authorizedUsers) && !in_array('all', $_SESSION['access'])) {
			die("unauthorized access");
		} 
		else {
			$data_a = $this->model->listPersons_version2(848);
			$data_b = $this->model->listPersons_version2(1064);
            $data = array_merge($data_a,$data_b);
		}
 		//impression dans excel
		require('helpers/excelPHP/PHPExcel.php');
		$excel = new PHPExcel();
    ...
    public function printretour(){
        $this->view->printretour();
        $this->render('printretour');
    }
    
    public function mn_printretour(){
		...
		if (!in_array($_SESSION['userid'], $this->authorizedUsers) && !in_array('all', $_SESSION['access'])) {
			...
			// Récupère tous les inscrits
            $data_a = $this->model->listPersons_version3(848);
			$data_b = $this->model->listPersons_version3(1064);
            $data = array_merge($data_a,$data_b);
		}
 		//impression dans excel
		require('helpers/excelPHP/PHPExcel.php');
