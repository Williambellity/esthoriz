...
class LogisticModel {
	...
	*/
	...
	public function getReducs($firmid) {
		$str = "";
		$query = "SELECT reducs_id FROM logistic_reduc WHERE firmid = :firmid";
		$sth = $this->db->prepare($query);
		$sth->bindParam(':firmid',$firmid, PDO::PARAM_INT);
		$sth->execute();
		...
		if($result) {
			...
			foreach ($result as $key => $v) {
				...
				if (!empty($result2)) {
					$str .= "LOGISTIC.system.recap.setReduc(".$result2['id'].",'".$result2['nom']."',".$result2['taux'].",'".$result2['applyTo']."');\n";
				}
...
/////////////////////////***********************************///////////////////////////////////////////				
				...
				foreach($elements as $element) {
					...
					if(!array_key_exists("",$element_t)) {
						foreach($element_t as $key=>$e) {
							$return['str'] .= "var t = [];\n";
							$return[$i][$element][$key] = $e;
							foreach(explode("#",$result['color']) as $color) {
								$file = "/upload/logistic/".strtolower($result['nom'])."/".$element."/".strtolower($e['nom'])."/".strtolower($color).".jpg";
	...
	public function get_firm_Lot_value($firmid,$lots,&$str) {
		...
		$sth->execute();
		$result = $sth->fetchAll();
		if(!empty($result[0]) && $result[0]['type_lot']!=0) {
			$result = $result[0];
			//$lots['any_record'] = true;
			$id = $result['type_lot'];
			$str .= "LOGISTIC.elements.lot.lots[".$id."].button.select(1);\n";
			$elements = Array(
				'chaise',
				'tabouret',
				'table',
			...
			foreach($elements as $element) {
				...
				if(array_key_exists($element,$lots[$id])) {
					foreach($lots[$id][$element] as $key=>$data) {
						$lots[$id][$element][$key]['more'] = isset($more[$key]) ? $more[$key]['nombre'] : 0;
						$str .= "LOGISTIC.elements.lot.lots[".$id."].accBasics['".$element."s']['".$data['nom']."'].button.select(".$lots[$id][$element][$key]['more'].");\n";
			...
			if(!empty($result['accessoires'])) {
				...
				foreach($accessoires as $accessoire) {
					...
					if($acc['for_all'] == 0) {
						$str .= "LOGISTIC.elements.accessoire.accessoires[".$accessoire['id']."].button.select(".$accessoire['nombre'].");\n";
					} else {
						$str .= "LOGISTIC.elements.accfixe.accfixes[".$accessoire['id']."].button.select(".$accessoire['nombre'].");\n";
					}
				}
			}
	...
	*/
		...
		foreach($result as $r) {
			...
			switch($r['categorie']) {
				...
				case 2:
					...
					break;
				case 3:
					$r['categorie'] = "Repas supplémentaires";
					break;
			}
	...
	public function get_firm_Option_value($firmid,&$options, &$str) {
		...
		if(!empty($result[0])) {
			...
			foreach($options as $key=>$option) {
				if(isset($options_default[$option['id']]) && $options_default[$option['id']]['nombre'] > 0) {
					...
					$str .= "LOGISTIC.elements.option.options[".$option['id']."].button.select(".$options_default[$option['id']]['nombre'].");\n";
				}
			}
		}
	}	
	...
	*/
	public function get_all_Stands() {
		...
		if(empty($result[0])) {
			...
			$return = "";
			foreach($result as $key=>$val) {
				$return .= "LOGISTIC.elements.stand.stands[".$val['id']."] = new LOGISTIC.elements.stand(".$val['id'].",".$val['prix'].",".$val['taille'].",".$val['nbstand'].");\n";
			}
			$result['str'] = $return;
			return $result;
		}		
	...
	public function get_firm_Stand_value($firmid,&$stands,&$str) {
		...
		if(!empty($result[0])) {
			foreach($stands as $key=>$stand) {
				if(is_array($stand) && $stand['id'] != $result[0]['type_stand']) {
				    ...
				    $str .= "LOGISTIC.elements.stand.stands[".$stand['id']."].button.select(1);\n";
	...
	*/
	...
	public function get_firm_Accessoire_value($firmid,&$accessoires,&$str) {
		...
		if(!empty($result[0])) {
			...
			foreach($accessoires as $key=>$accessoire) {
				if(isset($accessoires_default[$accessoire['id']]) && $accessoires_default[$accessoire['id']]['nombre'] > 0) {
					if($accessoire['for_all'] == 1) {
						$str .= "LOGISTIC.elements.accfixe.accfixes[".$accessoire['id']."].button.select(".$accessoires_default[$accessoire['id']]['nombre'].");\n";
					}
				}
			}
		}
	...
	public function get_firm_Color_value($firmid,&$str) {
		$query = "SELECT color FROM logistic_firm WHERE firmid = :firm";
		$sth = $this->db->prepare($query);
		$sth->bindParam(':firm',$firmid);
		$sth->execute();
		$result = $sth->fetchAll();
		if(!empty($result[0])) {
			$str .= "LOGISTIC.buttons.buttonColor.select('".$result[0]['color']."');\n";
		}
	}
	
	public function getAdding($firmid) {
