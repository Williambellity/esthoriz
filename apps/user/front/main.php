 ...
 *
 * @auteur	Fofif <Johan Dufau>
 * @version	$Id: apps/user/front/main.php 0003 13-05-2011 Fofif $
 */
...
class UserController extends WController {
	...
	public function login($nick, $pass, $remember) {
		...
		if (!empty($data)) {
			// DEBUT modif pour connexion contact
			include APP_DIR.'entreprise/front/model.php';
			$firmModel = new EntrepriseModel();
			$contactData = $firmModel->loadContact($data['id']);
			if (!empty($contactData)) {
				if (!empty($contactData['firmid'])) {
					$data['firm'] = $contactData;
				} else {
					WNote::info("Compte en cours de validation", "Pour l'heure, votre compte n'a pas encore été validé par notre équipe.<br />
						Vous recevrez bientôt un email de confirmation pour vous informer de votre inscription définitive.<br /><br />
						Toutefois, si l'attente devenait anormalement longue, n'hésitez pas à nous contacter par email à l'adresse forum@est-horizon.com", 'display');
					return;
				}
			}
			// FIN modif
