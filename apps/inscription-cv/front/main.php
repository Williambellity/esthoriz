...
class Inscription_CvController extends WController {
		...
		if ($data['mail']!=NULL){//!in_array(null, $data, true)) {
			...
			if (!empty($erreurs)) {
				...
				if ($this->model->writeInscription($data)) {
					...
					WNote::success(utf8_encode("CV déposé"), utf8_encode("Nous vous donnons rendez-vous le 2 décembre au Centre Prouvé.<br /><br />

					
					"), 'display');
					/*WNote::success("Informations enregistrées", "Nous vous remercions pour votre inscriptions. Vos informations ont bien �t�</br>
	...
	public function confirmation() {
		...
		if (!empty($email)) {
			...
			{
			WNote::success(utf8_encode("Inscription validée"), utf8_encode("Nous vous donnons rendez-vous le 2 décembre au Centre Prouvé.<br /><br />
