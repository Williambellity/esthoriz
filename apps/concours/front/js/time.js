...
(function(){ // Stand début de scope local
	...
	TIMELINE.events.event.prototype = {
		...
		constructor:function() {
            ...
            switch(this.z) {
                case 0:
                    $('#event_'+this.id).css({'top':'0px','height':'200px'});
                    $('#event_'+this.id+' .ti_flag').css({'height':'200px'});
                    break;

                case 1:
                    $('#event_'+this.id).css({'top':'40px','height':'180px'});
                    $('#event_'+this.id+' .ti_flag').css({'height':'180px'});
                    break;

                case 2:
                    $('#event_'+this.id).css({'top':'80px','height':'160px'});
                    $('#event_'+this.id+' .ti_flag').css({'height':'160px'});
                    break;

                case 3:
                    $('#event_'+this.id).css({'top':'120px','height':'140px'});
                    $('#event_'+this.id+' .ti_flag').css({'height':'140px'});
                    break;
				
				case 4:
                    $('#event_'+this.id).css({'top':'160px','height':'120px'});
                    $('#event_'+this.id+' .ti_flag').css({'height':'120px'});
                    break;
            }
...
(function(){ // Pages début de scope local
	...
	TIMELINE.system.line = {
		...
		display:function() {
            ...
            $.each(TIMELINE.events.event.listEvents,function(key,val) {
                if(TIMELINE.events.event.listEvents[key]!=undefined) {
                TIMELINE.events.event.listEvents[key].hideEvent();
                
                var temp1 = false;
                var temp2 = false;
                //for(var key_conc in TIMELINE.events.event.listEvents[key].concerne) {
                $.each(TIMELINE.events.event.listEvents[key].concerne,function(key_conc,val_conc) {
                    if(TIMELINE.events.event.listEvents[key].concerne[key_conc]!=undefined) {
                ...
                $.each(TIMELINE.events.event.listEvents[key].categories,function(key_cat,val_cat) {
                    if(TIMELINE.events.event.listEvents[key].categories[key_cat]!=undefined) {
            ...
            $.each(toDisplay,function(dkey,dval) {
                if(toDisplay[dkey]!=undefined) {
        ...
        allCategories:function() {
            ...
            $.each(TIMELINE.events.event.categories,function(c,val) {
                if(TIMELINE.events.event.categories[c]!=undefined) {
        ...
        allConcernes:function()  {
            ...
            $.each(TIMELINE.events.event.concerne,function(c,val) {
                if(TIMELINE.events.event.concerne[c]) {
