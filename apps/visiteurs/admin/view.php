...
class VisiteursAdminView extends WView {
	...
	public function liste($sortBy, $sens, $currentPage) {
		...
		$page = new Pagination($count, 5000, $currentPage, '/admin/visiteurs/'.$sort[0].'-'.strtolower($sort[1]).'-%d');
		$this->assign('pagination', $page->getHtml());
		
		$visit = $this->model->getVisiteurs(($currentPage-1)*5000, 5000, $sort[0], $sort[1] == 'ASC');
		$ctr=array();
		$i=0;
		while($i<30)
		{
	...
	public function autres($sortBy, $sens, $currentPage) {
		...
		// Gestion de la pagination (50 firms/page)
		include HELPERS_DIR.'pagination'.DS.'pagination.php';
		$page = new Pagination($count, 5000, $currentPage, '/admin/visiteurs/autres/autres-'.strtolower($sort[1]).'-%d');
		$this->assign('pagination', $page->getHtml());
		
		$visit = $this->model->getAutres(($currentPage-1)*5000, 5000,'autres', $sort[1] == 'ASC');
	...
	public function mailnonvalide($sortBy, $sens, $currentPage) {
		// AdminStyle Helper
		include HELPERS_DIR.'adminStyle'.DS.'adminStyle.php';
		$adminStyle = new AdminStyle(array('ecole', 'nom'), 'id', 'DESC');
		// Sorting vars
		...
		$count = $this->model->countVisiteursNonVal();
		print($count);
		
		// Gestion de la pagination (50 firms/page)
		include HELPERS_DIR.'pagination'.DS.'pagination.php';
		$page = new Pagination($count, 5000, $currentPage, '/admin/visiteurs/mailnonvalide/mailnonvalide-'.strtolower($sort[1]).'-%d');
		$this->assign('pagination', $page->getHtml());
		
		$visit = $this->model->getVisiteursNonVal(($currentPage-1)*5000, 5000,'ecole', $sort[1] == 'ASC');
