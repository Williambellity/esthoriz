...
class BrochureModel {
	...
	*/
	...
	private function updateBrochure($firmid, $data) {
		...
		$prep = $this->db->prepare('
			UPDATE brochure SET website = :website, creation_date = :creation_date, pdg = :pdg, ca = :ca, implantation = :implantation, effectifs = :effectifs, cadres = :cadres, age_moyen = :age_moyen, salaire_moyen = :salaire_moyen, sNb = :sNb, sProfil = :sProfil, sType = :sType, eNb = :eNb, eProfil = :eProfil, eType = :eType, marques = :marques, presentation = :presentation, savoir = :savoir, co = :co, f1 = :f1, f2=:f2, f3= :f3, f4=:f4, f5=:f5, contact = :contact, tel = :tel, etu= :etu, date = CURRENT_TIMESTAMP WHERE firmid = :firmid
		');
	...
	private function insertBrochure($firmid, $data) {
		...
		$prep = $this->db->prepare('
			...
			:eType, :marques, :presentation, :savoir,:co,:f1,:f2,:f3,:f4,:f5,:contact,:tel,:etu, CURRENT_TIMESTAMP)
		');
		$prep->bindParam(':firmid', $firmid, PDO::PARAM_INT);
		$prep->bindParam(':website', $data['website']);
		$prep->bindParam(':creation_date', $data['creation_date']);
