...
class BrochureModel {
	...
	*/
	...
	private function updateBrochure($firmid, $data) {
		...
		$prep = $this->db->prepare('
			UPDATE brochure SET website = :website, creation_date = :creation_date, pdg = :pdg, ca = :ca, implantation = :implantation, effectifs = :effectifs, marques = :marques, presentation = :presentation, savoir = :savoir, co = :co, f1 = :f1, f2= :f2, f3= :f3, f4= :f4, f5= :f5, contact = :contact, tel = :tel, etu= :etu, date = CURRENT_TIMESTAMP WHERE firmid = :firmid
		');
		//Verification que website est de la forme http://www. ...
		if (substr_count($data['website'], 'http') == 0){
		...
		$prep->bindParam(':co', $data['co']);
		$prep->bindParam(':f1', $data['f1']);
		$prep->bindParam(':f2', $data['f2']);
		$prep->bindParam(':f3', $data['f3']);
		$prep->bindParam(':f4', $data['f4']);
		$prep->bindParam(':f5', $data['f5']);
		$prep->bindParam(':contact', $data['contact']);
		$prep->bindParam(':tel', $data['tel']);
		$prep->bindParam(':etu', $data['etu']);
		$prep->bindParam(':firmid', $firmid, PDO::PARAM_INT);
		$prep->execute();
	...
	private function insertBrochure($firmid, $data) {
		...
		$prep = $this->db->prepare('
			...
			effectifs, marques, 
			presentation, savoir,co,f1,f2,f3,f4,f5,contact,tel,etu, date) VALUES (:firmid, :website, :creation_date, :pdg, 
			:ca, :implantation, :effectifs, :marques, :presentation, :savoir,:co,:f1,:f2,:f3,:f4,:f5,:contact,:tel,:etu, CURRENT_TIMESTAMP)
		');
		$prep->bindParam(':firmid', $firmid, PDO::PARAM_INT);
		$prep->bindParam(':website', $data['website']);
		$prep->bindParam(':creation_date', $data['creation_date']);
	...
	public function getBrochure($firmid) {
		...
		$prep = $this->db->prepare('
			...
			effectifs, marques, 
			presentation, savoir, co, f1, f2, f3, f4, f5, contact, tel, etu FROM brochure WHERE firmid = :firmid
		');		
		$prep->bindParam(':firmid', $firmid, PDO::PARAM_INT);
		$prep->execute();
		...
		if(!empty($out2)) {
			...
			"effectifs" => "", "marques" => "",
			"presentation" => "", "savoir" => "", "co" => "", "f1" => "", "f2" => "", "f3" => ""
			, "f4" => "", "f5" => "", "contact" => "", "tel" => "", "etu" => "");
