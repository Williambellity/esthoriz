 ...
 * 
 * @auteur	Fofif
 * @version	$Id: WSession.php 0003 02-08-2011 Fofif $
 */
...
class WSession {
	...
	private function reloadSession($userid, $cookie_hash) {
		...
		if ($query->rowCount() > 0) {
			...
			if ($cookie_hash == $this->generate_hash($data['nickname'], $data['password'])) {
				...
				$prep = $db->prepare('
					SELECT firmid, nom, prenom,  name
					FROM entreprises_contacts
					LEFT JOIN entreprises ON entreprises.id = firmid
					WHERE userid = :userid
				');
				$prep->bindParam(':userid', $userid, PDO::PARAM_INT);
				$prep->execute();
				$data['firm'] = $prep->fetch(PDO::FETCH_ASSOC);
				// FIN modif
	...
	public function loadUser($userid, $data) {
		...
		// DEBUT MODIF
		// Chargement des données de contact
		if (isset($data['firm'])) {
			$_SESSION['nom'] = $data['firm']['nom'];
			$_SESSION['prenom'] = $data['firm']['prenom'];
			$_SESSION['firmid'] = $data['firm']['firmid'];
			$_SESSION['firmname'] = $data['firm']['name'];
		} else {
			$_SESSION['nom'] = '';
			$_SESSION['prenom'] = '';
			$_SESSION['firmid'] = '';
