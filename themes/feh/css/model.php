...
class EtudiantsModel {
	...
	public function getBusInfo($idb ) {
		$prep = $this->db->prepare('
			SELECT idb, Lieu, Heure, Numero, max
			FROM fdh_bus
			WHERE idb = :idb
		');
		$prep->bindParam(':idb', $idb, PDO::PARAM_INT);
	...
	public function getPerson($id) {
		$prep = $this->db->prepare('
			SELECT id, ecole, nom, prenom, bus, pay_mod
			FROM fdh_inscrits
			WHERE id = :id
		');
	...
	public function registerPerson($data, $idb) {
		$prep = $this->db->prepare('
			INSERT INTO fdh_inscrits(ecole, nom, prenom, bus, pay_mod)
			VALUES (:ecole, :nom, :prenom, :bus, :paymod)
		');
		$prep->bindParam(':ecole', $_SESSION['userid']);
		$prep->bindParam(':nom', $data['nom']);
		$prep->bindParam(':prenom', $data['prenom']);
		$prep->bindParam(':bus', $data['bus']);
	...
	public function listPersons($ecole) {
		$prep = $this->db->prepare('
			SELECT fdh_inscrits.id AS id, nickname, bus, nom, prenom, DATE_FORMAT(heure_inscription, "%d-%m-%Y %H:%i") AS heure, pay_mod, ecole
			FROM fdh_inscrits
			LEFT JOIN users
